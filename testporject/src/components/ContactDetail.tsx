...
export default function ContactDetail({ id, onSave, onBack }: { id: string | null; onSave: () => void; onBack: () => void }) {
    ...
    const [form, setForm] = useState<Partial<Contact>>({ first_name: '', last_name: '', job_title: '', account_name: '', email: '', phone: '', mobile_phone: '', preferred_contact_method: 'Any', owner: 'Alex Wu' });
    ...
    const handleCommand = async (action: string) => {
        if (action === 'Save') {
            try { if (id) await contactsApi.update(id, form); else await contactsApi.create(form); onSave(); } catch (e: any) { setAlertTitle('Error'); setAlertMessage(e.message); setShowAlertDialog(true); }
    ...
    const initials = `${(form.first_name||'N')[0]}${(form.last_name||'N')[0]}`.toUpperCase();
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-6">
                            <div className="col-span-1 md:col-span-2 lg:col-span-3">
                                ...
                                <div className="flex flex-row items-center group py-1.5">
                                    <label className="text-[12px] text-slate-500 w-32 shrink-0">Preferred Method of Contact</label>
                                    <div className="flex-1 border-b border-slate-200 group-hover:border-slate-400 focus-within:border-[#0072c6] transition-colors relative">
                                        <select value={form.preferred_contact_method || 'Any'} onChange={e => updateField('preferred_contact_method', e.target.value)} className="bg-transparent py-1 text-[13px] text-slate-900 outline-none w-full appearance-none">
                                            <option>Any</option><option>Email</option><option>Phone</option><option>Mail</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
