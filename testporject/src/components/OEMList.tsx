...
export default function OEMList({ onSelect, onNew }: { onSelect: (id: string) => void; onNew: () => void }) {
  ...
  const fetchData = async () => { try { setLoading(true); setOems(await oemsApi.list()); } catch (e: any) { setAlertTitle("Error"); setAlertMessage(e.message); setShowAlertDialog(true); } finally { setLoading(false); } };
  useEffect(() => { fetchData(); }, []);
  ...
  const downloadTemplate = () => {
    const templateData = [{ 
      ...
      "Short Name EN": "Tesla", 
      "Short Name CN": "特斯拉", 
      "Full Name": "Tesla Motors Inc.", 
      "Associated Account": "Tesla China",
      "Application Coverage": "85%",
      "First Source": "Website",
      "Source List": "Website, Fair",
      "Modification Status": "Original"
    }];
    const worksheet = XLSX.utils.json_to_sheet(templateData);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Template");
    XLSX.writeFile(workbook, "OEM_Import_Template.xlsx");
  };
  ...
  const handleImport = async (file: File) => {
    try {
      ...
      const newOems = jsonData.map(row => ({
        name: row["OEM Name"] || row.name || 'Imported OEM',
        ...
        application_coverage: row["Application Coverage"] || row.application_coverage || '',
      ...
      if (newOems.length > 0) {
        setImportProgress({ current: 0, total: newOems.length });
        const chunkSize = 20;
        for (let i = 0; i < newOems.length; i += chunkSize) {
          const chunk = newOems.slice(i, i + chunkSize);
          await oemsApi.createMany(chunk);
          setImportProgress({ current: Math.min(i + chunkSize, newOems.length), total: newOems.length });
        }
        setShowImportModal(false);
        setSelectedFile(null);
        setAlertTitle("Success");
        setAlertMessage(`Successfully imported ${newOems.length} OEMs.`);
  ...
  const sortedData = [...oems].sort((a, b) => {
  ...
  const handleDelete = async () => { try { await oemsApi.delete(selectedIds); setOems(oems.filter(i => !selectedIds.includes(i.id))); setSelectedIds([]); setShowConfirmDialog(false); } catch (e: any) { setShowConfirmDialog(false); setAlertTitle("Error"); setAlertMessage(e.message); setShowAlertDialog(true); } };
  ...
  return (
    <div className="flex flex-col h-full bg-white relative">
      ...
      <PrintPreviewDialog 
        ...
        selectedItems={oems.filter(o => selectedIds.includes(o.id))} 
        titleField="name"
        entityName="OEMs"
        fields={[
          { id: 'name', label: 'OEM Name' },
          ...
          { id: 'application_coverage', label: 'Coverage' },
        ...
        <div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded shadow-2xl w-full max-w-[500px] flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <h2 className="text-[16px] font-semibold text-slate-900 flex items-center gap-2"><span className="material-symbols-outlined text-[#0072c6]">upload_file</span>Import OEMs</h2>
      ...
      <div className="px-6 py-4 flex items-center justify-between border-b border-white z-10 sticky top-0 bg-white shadow-sm">
        <div className="flex items-center gap-2"><h1 className="text-[20px] font-normal text-slate-900">Active OEMs</h1><span className="material-symbols-outlined text-slate-500 text-[18px] cursor-pointer">expand_more</span></div>
          ...
          <table className="w-full text-left border-collapse text-[13px] table-fixed">
            ...
            <tbody>{paginatedData.map((item, i) => (
              <tr key={item.id} onClick={() => onSelect(item.id)} className={`border-b border-slate-100 cursor-pointer ${selectedIds.includes(item.id) ? "bg-blue-50/50 hover:bg-blue-50" : `hover:bg-slate-50 ${i % 2 !== 0 ? "bg-slate-50/50" : ""}`} group`}>
                ...
                <td className="py-3 px-3 text-slate-800 truncate">{item.application_coverage}</td>
