...
export default function LeadDetail({ id, onSave, onBack }: { id: string | null; onSave: () => void; onBack: () => void }) {
    ...
    const [form, setForm] = useState<Partial<Lead>>({ first_name: '', last_name: '', topic: '', status: 'New', source: 'Web', rating: 'Warm', job_title: '', phone: '', email: '', company_name: '', website: '', owner: 'Alex Wu', stage_index: 0 });
    ...
    const handleCommand = async (action: string) => {
        if (action === 'Save') { try { if (id) await leadsApi.update(id, form); else await leadsApi.create(form); onSave(); } catch (e: any) { setAlertTitle('Error'); setAlertMessage(e.message); setShowAlertDialog(true); } }
        ...
        <div className="flex flex-col h-full bg-white">
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar p-4 md:p-6 bg-white">
                <div className="flex flex-col md:flex-row justify-between md:items-end border-b border-slate-200 gap-4 mb-6 mt-1">
                    ...
                    <div className="flex flex-wrap gap-8 pb-[8px]"><Hf label="Lead Source" value={form.source || ''} /><Hf label="Rating" value={form.rating || ''} /><div className="flex flex-col"><span className="text-[10px] uppercase font-bold tracking-wider text-slate-500 mb-1">Owner</span><div className="flex items-center gap-1.5"><div className="w-5 h-5 rounded-full bg-blue-100 text-[#005a9e] flex items-center justify-center text-[10px] font-bold">AW</div><span className="text-[13px] text-slate-900">{form.owner}</span></div></div></div>
                </div>
