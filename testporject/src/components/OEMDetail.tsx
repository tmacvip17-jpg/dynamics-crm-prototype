...
import { oemsApi, timelineApi } from '../lib/api';
import type { OEM, OEMInput, TimelineEntry } from '../lib/types';
import CustomSelect from './shared/CustomSelect';

export default function OEMDetail({ id, onSave, onBack }: { id: string | null; onSave: () => void; onBack: () => void }) {
    ...
    const [form, setForm] = useState<OEMInput>({ 
        name: '', 
        short_name_en: '', 
        short_name_cn: '', 
        full_name: '', 
    ...
    useEffect(() => {
        if (id) {
            setLoading(true);
            oemsApi.getById(id).then(data => { if (data) setForm(data); setLoading(false); }).catch(() => setLoading(false));
            timelineApi.listByEntity('oem', id).then(setTimeline).catch(() => {});
        }
    }, [id]);

    const updateField = (field: keyof OEMInput, value: any) => setForm(prev => ({ ...prev, [field]: value }));

    const handleCommand = async (action: string) => {
        if (action === 'Save' || action === 'Save & Close') {
            try {
                if (id) await oemsApi.update(id, form); else await oemsApi.create(form);
                onSave();
    ...
    const handleDelete = async () => { try { if (id) await oemsApi.delete([id]); setShowConfirmDialog(false); onBack(); } catch (e: any) { setShowConfirmDialog(false); setAlertTitle('Error'); setAlertMessage(e.message); setShowAlertDialog(true); } };
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar p-4 md:p-6 bg-white">
                <div className="flex flex-col md:flex-row justify-between md:items-end border-b border-slate-200 gap-4 mb-6 mt-1">
                    <div className="flex flex-col self-stretch justify-between">
                        <h1 className="text-xl font-normal text-slate-900 mb-5">{form.name || 'New OEM'}</h1>
                        <div className="flex gap-6 mt-auto">
                            <button className="text-[14px] font-semibold text-[#0072c6] pb-[7px] border-b-2 border-[#0072c6] mb-[-1px]">OEM</button>
                        </div>
                    </div>
                    <div className="flex flex-wrap gap-8 pb-[8px]">
                        <HdrField label="Coverage" value={form.application_coverage || '--'} />
                ...
                <div className="max-w-none">
                    {activeTab === 'info' && (
                        <div className="flex flex-col">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-3">
                                <FmField label="OEM Name *" value={form.name || ''} onChange={v => updateField('name', v)} />
                                ...
                                <FmField label="OEM Full Name" value={form.full_name || ''} onChange={v => updateField('full_name', v)} />
                                <FmField label="Associated Account" value={form.associated_account || ''} onChange={v => updateField('associated_account', v)} icon="business" />
                                <FmField label="Application Coverage" value={form.application_coverage || ''} onChange={v => updateField('application_coverage', v)} />
                    ...
                    {activeTab === 'timeline' && (
                        <div className="border border-slate-200 rounded flex flex-col h-[400px]">
                            ...
                            <div className="flex-1 overflow-y-auto p-3 bg-white text-center text-slate-500 text-[13px] pt-12">No timeline entries yet for this OEM.</div>
                        </div>
                    )}
                </div>
            </div>
