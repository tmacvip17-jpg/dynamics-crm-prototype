...
export default function CompetitorDetail({ id, onSave, onBack }: { id: string | null; onSave: () => void; onBack: () => void }) {
    ...
    const [form, setForm] = useState<Partial<Competitor>>({ name: '', website: '', ticker_symbol: '', strengths: '', weaknesses: '', owner: 'Alex Wu' });
    ...
    const handleCommand = async (action: string) => {
        if (action === 'Save') { try { if (id) await competitorsApi.update(id, form); else await competitorsApi.create(form); onSave(); } catch (e: any) { setAlertTitle('Error'); setAlertMessage(e.message); setShowAlertDialog(true); } }
        ...
        <div className="flex flex-col h-full bg-white">
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar p-4 md:p-6 bg-white">
                <div className="flex flex-col md:flex-row justify-between md:items-end border-b border-slate-200 gap-4 mb-6 mt-1">
                    ...
                    <div className="flex flex-wrap gap-8 pb-[8px]"><Hf label="Website" value={form.website||''} /><Hf label="Ticker Symbol" value={form.ticker_symbol||''} /><div className="flex flex-col"><span className="text-[10px] uppercase font-bold tracking-wider text-slate-500 mb-1">Owner</span><div className="flex items-center gap-1.5"><div className="w-5 h-5 rounded-full bg-blue-100 text-[#005a9e] flex items-center justify-center text-[10px] font-bold">AW</div><span className="text-[13px] text-slate-900">{form.owner}</span></div></div></div>
                </div>
