...
export default function AccountDetail({ id, onSave, onBack }: { id: string | null; onSave: () => void; onBack: () => void }) {
    ...
    useEffect(() => {
        if (id) {
            setLoading(true);
            accountsApi.getById(id).then(data => { if (data) setForm(data); setLoading(false); }).catch(() => setLoading(false));
            timelineApi.listByEntity('account', id).then(setTimeline).catch(() => {});
        }
    }, [id]);
    ...
    const handleDelete = async () => { try { if (id) await accountsApi.delete([id]); setShowConfirmDialog(false); onBack(); } catch (e: any) { setShowConfirmDialog(false); setAlertTitle('Error'); setAlertMessage(e.message); setShowAlertDialog(true); } };
    ...
    return (
        <div className="flex flex-col h-full bg-white">
            <div className="h-11 border-b border-slate-200 flex items-center px-4 gap-1 shrink-0 bg-white">
                ...
                <CmdBtn icon="share" label="Share" onClick={() => handleCommand('Share')} />
            </div>
            <div className="flex-1 overflow-y-auto no-scrollbar p-4 md:p-6 bg-white">
                <div className="flex flex-col md:flex-row justify-between md:items-end border-b border-slate-200 gap-4 mb-6 mt-1">
                    <div className="flex flex-col self-stretch justify-between">
                        <h1 className="text-xl font-normal text-slate-900 mb-5">{form.name || 'New Account'}</h1>
                        <div className="flex gap-6 mt-auto"><button className="text-[14px] font-semibold text-[#0072c6] pb-[7px] border-b-2 border-[#0072c6] mb-[-1px]">Account</button></div>
                    </div>
                    ...
                    {activeTab === 'info' && (
                        <div className="flex flex-col">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-3">
                                <FmField label="Account Name *" value={form.name || ''} onChange={v => updateField('name', v)} />
                                <FmField label="Main Phone" value={form.phone || ''} onChange={v => updateField('phone', v)} icon="call" />
                                <FmField label="Fax" value={form.fax || ''} onChange={v => updateField('fax', v)} />
                                <FmField label="Website" value={form.website || ''} onChange={v => updateField('website', v)} icon="language" />
                                <FmField label="Parent Account" value={form.parent_account || ''} onChange={v => updateField('parent_account', v)} icon="business" />
                                <FmField label="Ticker Symbol" value={form.ticker_symbol || ''} onChange={v => updateField('ticker_symbol', v)} />
                            </div>
                            <h2 className="text-[11px] font-bold uppercase tracking-wider text-slate-800 border-b border-slate-200 pb-2 mt-8 mb-4">Address</h2>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-3">
                                <FmField label="Street 1" value={form.street || ''} onChange={v => updateField('street', v)} />
                                <FmField label="City" value={form.city || ''} onChange={v => updateField('city', v)} />
                                <FmField label="State/Province" value={form.state || ''} onChange={v => updateField('state', v)} />
                                <FmField label="ZIP/Postal Code" value={form.zip || ''} onChange={v => updateField('zip', v)} />
                                <FmField label="Country/Region" value={form.country || ''} onChange={v => updateField('country', v)} />
                            </div>
                        </div>
                    )}
                    {activeTab === 'timeline' && (
                        <div className="border border-slate-200 rounded flex flex-col h-[400px]">
                            <div className="bg-slate-50 border-b border-slate-200 p-3">
                                <div className="flex gap-4 mb-3"><button className="text-[13px] font-semibold text-[#0072c6] pb-1 border-b-2 border-[#0072c6]">Posts</button><button className="text-[13px] text-slate-600 hover:text-slate-900 pb-1 border-b-2 border-transparent transition-colors">Activities</button><button className="text-[13px] text-slate-600 hover:text-slate-900 pb-1 border-b-2 border-transparent transition-colors">Notes</button></div>
                                <div className="relative mt-1"><input type="text" placeholder="Enter a note..." className="w-full border border-slate-300 rounded bg-white py-1.5 pl-3 pr-8 text-[13px] outline-none hover:border-slate-400 focus:border-[#0072c6] transition-colors shadow-sm" /><button className="absolute right-2 top-1/2 -translate-y-1/2 text-[#0072c6]"><span className="material-symbols-outlined text-[18px]">send</span></button></div>
                            </div>
                            <div className="flex-1 overflow-y-auto p-3 space-y-4 bg-white">
                                {timeline.length === 0 ? <div className="text-[13px] text-slate-500 text-center mt-8">No timeline entries yet.</div> :
                                timeline.map(t => <TlItem key={t.id} icon={t.icon} title={t.title} time={new Date(t.created_at).toLocaleDateString()} desc={t.description || ''} tags={t.tags || undefined} />)}
                            </div>
                        </div>
                    )}
                </div>
            ...
            <ConfirmationDialog isOpen={showConfirmDialog} title="Confirm Delete" message="Are you sure you want to delete this account?" onConfirm={handleDelete} onCancel={() => setShowConfirmDialog(false)} />
