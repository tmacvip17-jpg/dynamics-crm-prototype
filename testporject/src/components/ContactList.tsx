...
export default function ContactList({ onSelect, onNew }: { onSelect: (id: string) => void; onNew: () => void }) {
  ...
  const fetchData = async () => { try { setLoading(true); setContacts(await contactsApi.list()); } catch (e: any) { setAlertTitle("Error"); setAlertMessage(e.message); setShowAlertDialog(true); } finally { setLoading(false); } };
  useEffect(() => { fetchData(); }, []);
  ...
  const handleCommand = (action: string) => {
    ...
    else if (action === "Print") { if (selectedIds.length === 0) { setAlertTitle("Print"); setAlertMessage("Please select at least one contact to print."); setShowAlertDialog(true); } else setShowPrintPreview(true); }
  };
  ...
  const handleDelete = async () => { try { await contactsApi.delete(selectedIds); setContacts(contacts.filter(i => !selectedIds.includes(i.id))); setSelectedIds([]); setShowConfirmDialog(false); } catch (e: any) { setShowConfirmDialog(false); setAlertTitle("Error"); setAlertMessage(e.message); setShowAlertDialog(true); } };
  const allChecked = filteredContacts.length > 0 && selectedIds.length === filteredContacts.length;
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      <PrintPreviewDialog 
        ...
        selectedItems={contacts.filter(o => selectedIds.includes(o.id))} 
        titleField="first_name" // Will use first_name as title placeholder
        entityName="Contacts"
        fields={[
      ...
      <div className="px-6 py-4 flex items-center justify-between border-b border-white z-10 sticky top-0 bg-white shadow-sm">
        <div className="flex items-center gap-2"><h1 className="text-[20px] font-normal text-slate-900">My Active Contacts</h1><span className="material-symbols-outlined text-slate-500 text-[18px] cursor-pointer">expand_more</span></div>
        ...
        {loading ? <div className="flex items-center justify-center h-32 text-slate-500 text-[13px]">Loading...</div> : (
          <table className="w-full text-left border-collapse text-[13px]">
            <thead className="bg-white border-b-2 border-slate-200 sticky top-0 z-10 w-full"><tr>
              <th className="py-2.5 pl-4 pr-2 w-10 font-normal cursor-pointer" onClick={() => setSelectedIds(allChecked ? [] : filteredContacts.map(i => i.id))}><span className={`material-symbols-outlined text-[18px] ${allChecked ? "text-[#0072c6]" : "text-slate-400"}`}>{allChecked ? "check_box" : "check_box_outline_blank"}</span></th>
              <th className="py-2.5 px-3 font-semibold text-slate-600 uppercase tracking-widest text-[11px] cursor-pointer hover:bg-slate-50 transition-colors" onClick={() => handleSort('first_name')}>
                <div className="flex items-center gap-1">Full Name {sortConfig?.key === 'first_name' && <span className="material-symbols-outlined text-[14px] text-[#0072c6]">{sortConfig.direction === 'asc' ? 'arrow_upward' : 'arrow_downward'}</span>}</div>
              </th>
              <th className="py-2.5 px-3 font-semibold text-slate-600 uppercase tracking-widest text-[11px]">Job Title</th>
              <th className="py-2.5 px-3 font-semibold text-slate-600 uppercase tracking-widest text-[11px]">Company Name</th>
              <th className="py-2.5 px-3 font-semibold text-slate-600 uppercase tracking-widest text-[11px]">Email</th>
              <th className="py-2.5 px-3 font-semibold text-slate-600 uppercase tracking-widest text-[11px]">Business Phone</th>
            </tr></thead>
            <tbody>{filteredContacts.map((item, i) => (
              <tr key={item.id} onClick={() => onSelect(item.id)} className={`border-b border-slate-100 cursor-pointer ${selectedIds.includes(item.id) ? "bg-blue-50/50 hover:bg-blue-50" : `hover:bg-slate-50 ${i % 2 !== 0 ? "bg-slate-50/50" : ""}`} group`}>
                ...
                <td className="py-3 px-3 text-slate-800">{item.job_title}</td>
                <td className="py-3 px-3 text-[#0072c6]">{item.account_name}</td>
      ...
      <div className="h-10 border-t border-slate-200 flex items-center justify-between px-6 text-[12px] text-slate-500 bg-white shrink-0 mt-auto">
        <div>1 - {filteredContacts.length} of {filteredContacts.length} (Filtered)</div>
