...
import { oesApi, timelineApi } from '../lib/api';
import type { OES, OESInput, TimelineEntry } from '../lib/types';
import CustomSelect from './shared/CustomSelect';

export default function OESDetail({ id, onSave, onBack }: { id: string | null; onSave: () => void; onBack: () => void }) {
    ...
    const [form, setForm] = useState<OESInput>({ 
        name: '', 
        short_name_en: '', 
        short_name_cn: '', 
        full_name: '', 
    ...
    useEffect(() => {
        if (id) {
            setLoading(true);
            oesApi.getById(id).then(data => { if (data) setForm(data); setLoading(false); }).catch(() => setLoading(false));
            timelineApi.listByEntity('oes', id).then(setTimeline).catch(() => {});
        }
    }, [id]);

    const updateField = (field: keyof OESInput, value: any) => setForm(prev => ({ ...prev, [field]: value }));

    const handleCommand = async (action: string) => {
        if (action === 'Save' || action === 'Save & Close') {
            try {
                if (id) await oesApi.update(id, form); else await oesApi.create(form);
                onSave();
    ...
    const handleDelete = async () => { try { if (id) await oesApi.delete([id]); setShowConfirmDialog(false); onBack(); } catch (e: any) { setShowConfirmDialog(false); setAlertTitle('Error'); setAlertMessage(e.message); setShowAlertDialog(true); } };
    ...
    return (
        <div className="flex flex-col h-full bg-white">
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar p-4 md:p-6 bg-white">
                <div className="flex flex-col md:flex-row justify-between md:items-end border-b border-slate-200 gap-4 mb-6 mt-1">
                    <div className="flex flex-col self-stretch justify-between">
                        <h1 className="text-xl font-normal text-slate-900 mb-5">{form.name || 'New OES'}</h1>
                        <div className="flex gap-6 mt-auto">
                            <button className="text-[14px] font-semibold text-[#0072c6] pb-[7px] border-b-2 border-[#0072c6] mb-[-1px]">OES</button>
                        </div>
                    </div>
                    <div className="flex flex-wrap gap-8 pb-[8px]">
                        <HdrField label="Battery Makers" value={form.battery_makers || '--'} />
                        <HdrField label="Mod Status" value={form.data_mod_status || '--'} />
                ...
                <div className="max-w-none">
                    {activeTab === 'info' && (
                        <div className="flex flex-col">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-3">
                                <FmField label="OES Name *" value={form.name || ''} onChange={v => updateField('name', v)} />
                                <FmField label="English Short Name" value={form.short_name_en || ''} onChange={v => updateField('short_name_en', v)} />
                                <FmField label="Chinese Short Name" value={form.short_name_cn || ''} onChange={v => updateField('short_name_cn', v)} />
                                <FmField label="OES Full Name" value={form.full_name || ''} onChange={v => updateField('full_name', v)} />
                                <FmField label="Associated Account" value={form.associated_account || ''} onChange={v => updateField('associated_account', v)} icon="business" />
                                <FmField label="Battery Makers" value={form.battery_makers || ''} onChange={v => updateField('battery_makers', v)} />
                                <FmField label="First Data Source" value={form.first_data_source || ''} onChange={v => updateField('first_data_source', v)} />
                                <FmField label="Data Source List" value={form.data_source_list || ''} onChange={v => updateField('data_source_list', v)} />
                                <FmField label="Mod Status" value={form.data_mod_status || ''} isSelect options={['Original', 'Modified', 'Deprecated']} onChange={v => updateField('data_mod_status', v)} />
                    ...
                    {activeTab === 'timeline' && (
                        <div className="border border-slate-200 rounded flex flex-col h-[400px]">
                            ...
                            <div className="flex-1 overflow-y-auto p-3 bg-white text-center text-slate-500 text-[13px] pt-12">No timeline entries yet for this OES.</div>
                        </div>
                    )}
                </div>
            </div>
            ...
            <ConfirmationDialog isOpen={showConfirmDialog} title="Confirm Delete" message="Are you sure you want to delete this entry?" onConfirm={handleDelete} onCancel={() => setShowConfirmDialog(false)} />
